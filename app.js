...
app.get("/upsertDB", async (req, res, next) => {
  ...
  for (course of courses) {
    ...
    await Course.findOneAndUpdate(
      { subject, coursenum, section, term, times },
      course,
      { upsert: true }
    );
  }
...
app.get("/courses/byName", async (req, res, next) => {
  res.render("courselist");
});

app.post("/courses/byName", async (req, res, next) => {
  const keyword = req.body.name;
  const courses = await Course.find({ name: { $regex: keyword } });
  res.locals.courses = courses;
  res.render("courselist");
});
